...
document.querySelectorAll('.faq__item .collapsible-toggle[aria-expanded="false"]').forEach(item => {
  item.addEventListener('click', function(){
    const parent = item.closest('.faq__wrapper');
    const allSiblings = Array.from(parent.children);
    const siblings = allSiblings.filter(element => element !== item.closest('.faq__item'));
    siblings.forEach(sibling => {
      const toggle = sibling.querySelector('.collapsible-toggle[aria-expanded="true"]');
      if (toggle) {
